...
// ИИ (зум камеры, по прямому запросу пользователя): render() рисует всю
// сцену внутри одного ctx.scale(State.camera.zoom, ...) (см. render() в
// 08-render.js) — поэтому worldToScreen/screenToWorld ВЫШЕ намеренно НЕ
// учитывают zoom и остаются "дозумовыми" координатами: canvas сам
// растягивает их на экране через свою трансформацию. Если бы эти функции
// сами умножали на zoom, позиции и размеры при отрисовке масштабировались
// бы дважды (сама функция + ctx.scale).
//
// А вот события мыши/тача (e.clientX/clientY) приходят в НАСТОЯЩИХ
// экранных пикселях — уже ПОСЛЕ визуального зума. Чтобы правильно
// переводить такую точку в мировые координаты (и обратно — для
// хит-тестов рамкой выделения/тапом по юниту), нужна версия, которая
// zoom учитывает явно. Используется только в 09-input.js.
function worldToScreenZoomed(x, y) {
  const z = State.camera.zoom;
  return { x: (x - State.camera.x) * z, y: (y - State.camera.y) * z };
}
function screenToWorldZoomed(sx, sy) {
  const z = State.camera.zoom;
  return { x: sx / z + State.camera.x, y: sy / z + State.camera.y };
}

// Сколько мирового пространства сейчас видно на экране по каждой оси —
// нужно везде, где раньше просто брали canvas.logicalWidth/Height как
// границу отсечения при отрисовке (см. 08-render.js): под ctx.scale(zoom)
// видимая "дозумовая" область растёт при zoom<1 и сжимается при zoom>1.
function visibleWorldWidth() {
  return canvas.logicalWidth / State.camera.zoom;
}
function visibleWorldHeight() {
  return canvas.logicalHeight / State.camera.zoom;
}
