...
const WALL_PERIMETER_MARGIN = 90; // px отступ периметра от крайних построек
const ENEMY_WALL_BUILD_PER_TICK = 4; // сколько стен максимум ставим за один decision-тик (не "все за раз одним взрывом кредитов" — ограничено и кредитами, и этим числом)
...
function enemyWallStepClustered() {
  ...
  for (const p of perimeterPoints) {
    ...
    if (!isEnemyBuildPlacementValid("wall", p.x, p.y)) continue; // место занято (своим зданием/уже стеной/чужим) — пропускаем именно эту точку периметра, не всю попытку
