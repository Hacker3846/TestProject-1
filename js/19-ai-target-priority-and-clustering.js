...
// ПРАВКА (ИИ №48, по прямому запросу пользователя: "ИИ владеет только
// стенами... не умеет грамотно вложиться"): было 10 — до 400 кредитов
// стен за ОДИН decision-тик (0.8-2.6с на hard/easy), что зачастую
// превышало весь доход ИИ в секунду и превращало стены в самый быстрый
// сток кредитов в игре, забивая деньги, которые иначе пошли бы в армию.
// Снижено до 3 (120кр/тик максимум) — периметр всё ещё достраивается
// (просто за большее число тиков), но перестаёт "проглатывать" всплеск
// накопленных кредитов мгновенно. Доп. смягчение приоритета стен вообще —
// см. 26-ai-defense-economy-throttle.js (порог "есть ли причина
// защищаться" поверх этого числа).
const ENEMY_WALL_BUILD_PER_TICK = 3; // сколько стен максимум ставим за один decision-тик
...
function enemyWallStepClustered() {
  ...
  // БАГФИКС (найден при повторных проверках после первой правки "не
  // оставляя зазора между заборами"): изначально здесь стоял ранний выход
  // "if (wallCount >= target) return" ДО пересчёта bounding box/периметра,
  // где wallCount — ГЛОБАЛЬНОЕ число стен ИИ на карте, а target — число
  // точек ТЕКУЩЕГО периметра. Оба варианта такого сравнения (в начале
  // функции и позже — внутри цикла построения) давали одну и ту же ошибку:
  // если периметр СМЕСТИЛСЯ (здание разрушено с одной стороны базы,
  // построено новое с другой — bounding box изменился), количество стен
  // где-то на карте могло случайно сравняться или превысить число точек
  // НОВОГО периметра, хотя сами эти стены стоят в других координатах и
  // новый периметр по факту остаётся голым — функция выходила/обрывала
  // цикл, ничего не построив, и дыра оставалась бы навсегда.
  // Исправление: считаем прогресс ТОЛЬКО относительно точек ЭТОГО
  // периметра (coveredOnThisPerimeter — сколько из perimeterPoints уже
  // заняты стеной), а не по общему числу стен на карте. enemyWallTarget()
  // по построению равна min(ENEMY_WALL_TARGET_CAP, perimeterPoints.length)
  // — то есть это и есть верхняя граница кредитов, которые разумно тратить
  // на этот периметр целиком; из неё вычитаем уже покрытое, чтобы понять,
  // сколько ЕЩЁ можно/нужно построить, и берём меньшее с ENEMY_WALL_BUILD_PER_TICK
  // как лимит на один decision-тик.
  const capTarget = enemyWallTarget();
  const coveredOnThisPerimeter = perimeterPoints.filter(p => enemyBuildings().some(b =>
    b.type === "wall" && Math.hypot(b.x - p.x, b.y - p.y) < step * 0.5
  )).length;
  const remainingByCap = Math.max(0, capTarget - coveredOnThisPerimeter);
  const buildLimit = Math.min(ENEMY_WALL_BUILD_PER_TICK, remainingByCap);

  let built = 0;
  for (const p of perimeterPoints) {
    if (built >= buildLimit) break;
