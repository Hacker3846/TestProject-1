...
// путь.
// ПРАВКА (по прямому запросу пользователя: "ии строит стены базу как то
// тупо, надо чтобы он кучковал здания... не оставляя зазора между
// заборами") — раньше кольца доходили до 480-560px от штаба, из-за чего
// база физически не могла быть компактной, а периметр стен вокруг такого
// разброса получался огромным и не влезал в лимит числа стен (см. правки
// в 18-walls.js/19-...js). Кольца сжаты примерно вдвое по max и по step —
// здания теперь ложатся плотной кучей рядом со штабом, зональное деление
// (тыл/фланги/фронт) само по себе не трогается, меняется только то, КАК
// ДАЛЕКО может уйти застройка каждой зоны.
// ПРАВКА (найдено при повторной проверке): исходные значения start ниже
// оказались МЕНЬШЕ, чем реально нужно, чтобы здание вообще не
// перекрывалось со штабом (commandCenter 3x3 тайла = half 48px).
// economy-здания (powerPlant/refinery, 2x2 = half 32px) требуют минимум
// 48+32=80px от центра штаба до центра здания, иначе коллизия
// гарантирована. production-здания (barracks 2x3/warFactory 3x2/airfield
// 3x3, half до 48px) требуют минимум 48+48=96px. С прежними start=55/70
// (меньше этих порогов) самое первое кольцо почти всегда давало ТОЛЬКО
// невалидные точки — не ломает игру (isEnemyBuildPlacementValid просто
// отклоняет и код переходит на следующее кольцо/угол), но тратит впустую
// ENEMY_PLACEMENT_RING_ATTEMPTS попыток на пустое кольцо и слегка
// задерживает первую постройку. start подняты так, чтобы первое кольцо
// каждой зоны уже было геометрически достижимо для типичного здания той
// зоны.
const AI_ZONE_RINGS = {
  defense:    { start: 130, step: 40, max: 300 },
  production: { start: 100, step: 38, max: 260 },
  economy:    { start: 85,  step: 34, max: 200 },
  default:    { start: 100, step: 40, max: 280 },
};
...
/* ==================================================================
   ...
   ================================================================== */
// ПРАВКА (по прямому запросу пользователя: "турели чем больше тем лучше") —
// раньше кап был 5, а цель росла как substantial/2 (округление вверх), т.е.
// на типичной базе из 6-8 небоевых зданий выходило 3-4 турели, и почти
...
// стенами... не умеет грамотно вложиться"): было множитель 1.3-1.4 и кап
// 22 (турелей БОЛЬШЕ, чем прочих построек) — 22×350кр=7700кр статичной
// обороны наравне (а то и больше) со стенами съедали экономику, которая
// иначе шла бы в армию/технику. Множитель понижен до 0.8 (турелей теперь
// МЕНЬШЕ числа прочих построек — оборона дополняет базу, а не доминирует
// над ней), кап — до 10. Минимум по-прежнему 2 — совсем маленькая база
// сразу получает хоть какую-то оборону. Дополнительный, threat-зависимый
// множитель поверх этого — см. 26-ai-defense-economy-throttle.js.
const ENEMY_TURRET_TARGET_CAP = 10;
function enemyTurretTarget() {
  const substantial = enemyBuildings().filter(b => b.type !== "wall" && b.type !== "turret").length;
  return Math.min(ENEMY_TURRET_TARGET_CAP, Math.max(2, Math.ceil(substantial * 0.8)));
}
