...
function drawGroundTexture(t) {
  ...
  // лёгкий градиент "линии рельефа" вместо чистой заливки, чтобы карта
  // читалась как местность, а не как таблица
  ctx.save();
  const grad = ctx.createLinearGradient(0, 0, 0, canvas.logicalHeight);
  ...
  ctx.lineWidth = 1;
  for (let x = offX; x < canvas.logicalWidth; x += ts) {
    ctx.beginPath(); ctx.moveTo(x + 0.5, 0); ctx.lineTo(x + 0.5, canvas.logicalHeight); ctx.stroke();
  }
  for (let y = offY; y < canvas.logicalHeight; y += ts) {
    ctx.beginPath(); ctx.moveTo(0, y + 0.5); ctx.lineTo(canvas.logicalWidth, y + 0.5); ctx.stroke();
  }

  // редкие декоративные пятна "травы/грязи" на пересечениях крупной сетки —
  // детерминированно по координате тайла (без Math.random в рендере, иначе
  // будет мерцать каждый кадр), чтобы карта не выглядела совсем стерильной
  const bigTs = ts * 3;
  const startCol = Math.floor(State.camera.x / bigTs) - 1;
  const startRow = Math.floor(State.camera.y / bigTs) - 1;
  const cols = Math.ceil(canvas.logicalWidth / bigTs) + 2;
  const rows = Math.ceil(canvas.logicalHeight / bigTs) + 2;
  ctx.fillStyle = "rgba(90,120,50,0.05)";
  for (let dc = 0; dc < cols; dc++) {
    for (let dr = 0; dr < rows; dr++) {
      const col = startCol + dc, row = startRow + dr;
      const h = ((col * 928371 + row * 123457) >>> 0) % 100;
      if (h > 14) continue;
      const wx = col * bigTs + (h % 7) * 6, wy = row * bigTs + ((h * 3) % 7) * 6;
      const s = worldToScreen(wx, wy);
      if (s.x < -20 || s.y < -20 || s.x > canvas.logicalWidth + 20 || s.y > canvas.logicalHeight + 20) continue;
      ctx.beginPath(); ctx.arc(s.x, s.y, 10 + (h % 5), 0, Math.PI * 2); ctx.fill();
    }
  }
}
...
function drawResourceNode(r, t) {
  ...
  if (s.x < -24 || s.y < -24 || s.x > canvas.logicalWidth + 24 || s.y > canvas.logicalHeight + 24) return;
...
function drawBuilding(b, t) {
  ...
  if (s.x < -w - 20 || s.y < -h - 20 || s.x > canvas.logicalWidth + w + 20 || s.y > canvas.logicalHeight + h + 20) return;
...
function drawFogBuildingMemory(t) {
  ...
  Object.values(State.fog.buildingGhosts).forEach(ghost => {
    ...
    const w = def.w * GameConfig.tileSize, h = def.h * GameConfig.tileSize;
    if (s.x < -w || s.y < -h || s.x > canvas.logicalWidth + w || s.y > canvas.logicalHeight + h) return;
    ctx.save();
...
function drawFogOverlay() {
  ...
  const colsOnScreen = Math.ceil(canvas.logicalWidth / fts) + 2;
  const rowsOnScreen = Math.ceil(canvas.logicalHeight / fts) + 2;
...
function renderRemoteGhosts() {
  ...
  Object.values(State.remoteGhosts).forEach(pdata => {
    ...
    Object.values(pdata.buildings || {}).forEach(b => {
      const s = worldToScreen(b.x, b.y);
      if (s.x < -40 || s.y < -40 || s.x > canvas.logicalWidth + 40 || s.y > canvas.logicalHeight + 40) return;
      const def = BuildingDefs[b.type];
      if (!def) return;
      const w = def.w * GameConfig.tileSize, h = def.h * GameConfig.tileSize;
    ...
    Object.values(pdata.units || {}).forEach(u => {
      const s = worldToScreen(u.x, u.y);
      if (s.x < -20 || s.y < -20 || s.x > canvas.logicalWidth + 20 || s.y > canvas.logicalHeight + 20) return;
      ctx.fillStyle = "#5a6a8a";
      ctx.beginPath();
