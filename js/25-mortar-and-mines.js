...
//
// ПРАВКА (прямой запрос пользователя: "Мины слишком ИМБа и по моему Мортира
// тоже, ограничь их чтобы битвы не были бесконечными, и пусть мины летающие
// 'авиа' технику не трогают"):
//  6) Мортира: attackCooldown 4000 -> 7000 (стреляет заметно реже; по
//     уточнению пользователя — именно частоту, не урон/радиус/цену/лимит).
//  7) Мина: attackDamage MORTAR_DAMAGE*2 (68) -> MORTAR_DAMAGE*1.25 (~43)
//     (по уточнению пользователя — именно урон, не количество; лимит мин
//     на игрока как был не завёден, так и остаётся не завёден).
//  8) Мина игнорирует летающих (UnitDefs.*.flying — aircraft/gunship): не
//     детонирует от их приближения и не задевает их своим splash, даже
//     если авиация оказалась в радиусе взрыва, вызванного наземным
//     юнитом. См. правки внутри updateLandmines() в Части 2.
...
// ПРАВКА (прямой запрос пользователя: "Мортира тоже ИМБа, ограничь чтобы
// битвы не были бесконечными" -> уточнение "увеличить cooldown, реже
// стреляет") — attackCooldown поднят с 4000 до 7000. Урон/радиус/лимит "3
// на игрока" не трогаются: пользователь явно выбрал именно "реже стреляет",
// не "слабее бьёт". 7000 (было 4000) — почти вдвое реже, ощутимо снижает
// суммарный splash-DPS мортиры за бой, сохраняя силу одного попадания.
BuildingDefs.mortar = {
  label: "Мортира", cost: mortarCostForDamage(MORTAR_DAMAGE), hp: 150, maxHp: 150, w: 2, h: 2,
  producesPower: 0, powerUse: 25, buildTime: 9000,
  canAttack: true, attackRange: 240, attackDamage: MORTAR_DAMAGE, attackCooldown: 7000,
  ...
  // ПРАВКА выше (см. коммент над BuildingDefs.mortar): было 4000, поднято
  // до 7000 по прямому запросу пользователя — мортира и так самое дорогое/
  // мощное оборонительное здание в игре (splash по площади), а бой с
  // несколькими мортирами на 4000ms кулдауне превращался в непрерывный
  // шквал AoE, из-за которого сражения не заканчивались. Логика патча
  // "поймать момент выстрела" (Часть 1 ниже, patchUpdateDefensiveStructures-
  // ForMortarSplash) читает attackCooldown ДИНАМИЧЕСКИ через def.attackCooldown
  // на каждый тик — отдельно ничего патчить не нужно, значение подхватится
  // само.
  sightRange: 260,
...
// задели площадным уроном соседней мортиры/AoE).
//
// ПРАВКА (прямой запрос пользователя: "Мины слишком ИМБа, ограничь их" ->
// уточнение "ограничь урон") — attackDamage мины снижен с x2 MORTAR_DAMAGE
// (68) до x1.25 MORTAR_DAMAGE (~43). Изначальный x2 был буквальной
// реализацией самой первой формулировки запроса ("урон x2 больше"), но на
// практике невидимая одноразовая ловушка без лимита количества у игрока,
// бьющая сильнее собственно мортиры (34) в 2 раза — оказалась сильнейшим
// источником урона в игре при том, что её вообще нельзя увидеть и
// подготовиться. x1.25 сохраняет "ловушка бьёт больнее одного выстрела
// мортиры" (43 > 34), но перестаёт быть буквально вдвое опаснее самого
// дорогого оборонительного здания в игре.
...
const LANDMINE_DAMAGE_MULTIPLIER = 1.25; // было 2 — см. правку выше
BuildingDefs.landmine = {
  ...
  attackDamage: Math.round(MORTAR_DAMAGE * LANDMINE_DAMAGE_MULTIPLIER), // было MORTAR_DAMAGE*2 (68), теперь ~43 — см. правку выше
  sightRange: 60,
...
function updateLandmines() {
  Object.values(State.buildings).forEach(mine => {
    ...
    // логике Clash of Clans: ловушки триггерятся войсками, не стенами/
    // постройками противника).
    //
    // ПРАВКА (прямой запрос пользователя: "пусть мины летающие 'авиа'
    // технику не трогают") — летающие юниты (UnitDefs[...].flying, см.
    // aircraft/gunship в 01-config-state.js) закопанную наземную мину
    // физически не задевают: она зарыта в землю и реагирует только на
    // то, что идёт по земле. Проверяем u.flying ЗДЕСЬ же, в самом отборе
    // триггера — если единственный юнит в радиусе летающий, мина просто не
    // взрывается вовсе (не тратится впустую и не бьёт по площади заодно
    // соседнюю пехоту, если авиация пролетела мимо в компании с пехотой —
    // см. также фильтр в самом splash ниже на случай, если наземный
    // триггер найден, но рядом в радиусе поражения оказался ещё и борт).
    let trigger = null;
    for (const u of Object.values(State.units)) {
      if (u.hp <= 0) continue;
      if (!isEnemyOf(mine.ownerId, u.ownerId)) continue;
      if (u.flying) continue; // авиация не детонирует мину — см. правку выше
    ...
    Object.values(State.units).forEach(u => {
      ...
      if (u.flying) return; // ПРАВКА: авиация не задевается взрывом мины — см. коммент у выбора trigger выше
